...
function resolveBundledConfigs(context) {
...
function log(messageOrCallback, data) {
    ...
    if (isDebugMode) {
        ...
        const message = typeof messageOrCallback === "function"
            ? messageOrCallback()
            : messageOrCallback;
...
function activate(context) {
    ...
    const provider = vscode.languages.registerDocumentSemanticTokensProvider(languageMap, new SemanticTokensProvider(cache), LEGEND);
    context.subscriptions.push(provider);
    // setup the selection range provider
    const selectionProvider = vscode.languages.registerSelectionRangeProvider(languageMap, new SelectionRangeProvider(cache));
    ...
    const reload = vscode.commands.registerCommand("tree-sitter-vscode-implementation.reload", () => {
...
async function initLanguage(config) {
    ...
    await web_tree_sitter_1.Parser.init().catch();
...
function addPosition(range, pos) {
...
function splitToken(token) {
    ...
    if (start.line != end.line) {
        ...
        for (let i = 1; i < lineDiff; i++) {
            const middleToken = {
                range: new vscode.Range(new vscode.Position(start.line + i, 0), new vscode.Position(start.line + i, maxLineLength)),
                type: token.type,
                modifiers: token.modifiers,
            };
...
class SemanticTokensProvider {
    ...
    matchesToTokens(lang, matches, cancellationToken) {
        const unsplitTokens = matches
            ...
            if (lang.semanticTokenTypeMappings &&
                Object.prototype.hasOwnProperty.call(lang.semanticTokenTypeMappings, originalCaptureName)) {
                ...
                log(() => {
                    return `Applied type mapping for original name: ${originalCaptureName} → ${mapping.targetTokenType}${mapping.targetTokenModifiers &&
            ...
            else if (lang.semanticTokenTypeMappings &&
                Object.prototype.hasOwnProperty.call(lang.semanticTokenTypeMappings, type)) {
                const mapping = lang.semanticTokenTypeMappings[type];
                type = mapping.targetTokenType;
                modifiers = mapping.targetTokenModifiers ?? [];
                log(() => {
                    return `Applied type mapping for base type: ${type} → ${mapping.targetTokenType}${mapping.targetTokenModifiers &&
                        mapping.targetTokenModifiers.length > 0
                        ? ` with modifiers: ${mapping.targetTokenModifiers.join(", ")}`
                        : ""}`;
                });
    ...
    async getInjection(match, cancellationToken) {
        ...
        if (lang === undefined)
            return null;
        // determine capture
        const injectionContent = match.captures.find((capture) => capture.name === "injection.content");
